				...
				<div class="page-header">
					...
					<?php
						...
						if (empty($login) or empty($password) or empty($email)) {
							echo ("<div class='alert alert-warning'><strong>Внимание! </strong>Вы ввели не всю информацию. Вернитесь на страницу регистрации и заполните все поля!</div><form action='/register.php'><button class ='btn btn-success btn-group' onlick='return false;'>Вернуться назад</button></form>");
						} else {
							...
							if (!empty($myrow['id'])) {
								echo "<div class='alert alert-warning'>Извините, введённый вами логин уже зарегистрирован. Введите другой логин.</div><form action='/register.php'><button class ='btn btn-success btn-group' onlick='return false;'>Вернуться назад</button></form><br>";
								} else {
									if (!empty($myrow2['id'])) {
										echo "<div class='alert alert-warning'>Извините, введённый вами E-Mail уже зарегистрирован. Введите другой E-Mail.</div><form action='/register.php'><button class ='btn btn-success btn-group' onlick='return false;'>Вернуться назад</button></form><br>";
										} else {
											$result3 = mysql_query ("INSERT INTO users (login,password,email) VALUES('$login','$md5password','$email')");
											if ($result3=='TRUE')
												{
													echo "<div class='alert alert-success'><strong>Вы зарегистрированы! </strong>Теперь вы можете зайти на сайт.</div><br> <form action='/login.php'><button class ='btn btn-success btn-group' onlick='return false;'>Страница авторизации</button></form>";
												}else {
													echo "<div class='alert alert-danger'><strong>Ошибка! </strong>Вы не зарегистрированы.</div><form action='/register.php'><button class ='btn btn-success btn-group' onlick='return false;'>Вернуться назад</button></form>";
												}
											}
									}
							}
