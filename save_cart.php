            ...
            <div class="col-md-6">
				<div class="page-header">
					...
					<?php
					 ...
					 if (empty($count)) 
						{
							echo ("<div class='alert alert-warning'><strong>Внимание! </strong>Ваша корзина пуста.</div><form action='/'><button class ='btn btn-success btn-group' onlick='return false;'>Вернуться на главную</button></form>");
						}else{
					if (empty($adress)) 
						{
							echo ("<div class='alert alert-warning'><strong>Внимание! </strong>Вы не ввели адрес доставки.</div><form action='/cartconfirm.php'><button class ='btn btn-success btn-group' onlick='return false;'>Вернуться назад</button></form>");
						}else{
					if (empty($phone)) 
						{
							echo ("<div class='alert alert-warning'><strong>Внимание! </strong>Вы не ввели номер телефона.</div><form action='/cartconfirm.php'><button class ='btn btn-success btn-group' onlick='return false;'>Вернуться назад</button></form>");
						}else{
						$summa = stripslashes($summa);
						...
						$titles = trim($titles);
						include ("bd.php");
						$result = mysql_query ("INSERT INTO cart (summ,user,count,adress,phone,ids,titles) VALUES('$summa','$user','$count','$adress','$phone','$ids','$titles')");
						$cart_id = mysql_insert_id();
						//Емейл покупателю
						$to  = $email ; 
						$subject = 'Ваш заказ №'.$cart_id.' в магазине Суши экспресс'; 
						$message = ' 
						<html> 
								Здравствуйте, ' .$login. '! Вы совершили заказ в магазине Суши Экспресс.
								Вы приобрели: <b>'.$titles.'</b><br><b>Сумма вашего заказа составляет: '.$summa.' рублей.</b><br>
								Скоро по указаному вами номеру телефона '.$phone.' с вами свяжется наш оператор, и уточнит некоторые детали заказа. Приятного аппетита!
						</html>'; 
						$headers  = "Content-type: text/html; charset=utf-8 \r\n"; 
						$headers .= "From: Суши Экспресс <vh128960@vh2.eurobyte.ru>\r\n";  
						mail($to, $subject, $message, $headers); 
						//Емейл админу
						$to2  = "madkort@gmail.com" ; 
						$subject2 = 'Новый заказ №'.$cart_id.' в магазине Суши Экспресс'; 
						$message2 = ' 
						<html> 
								<b>Информация о заказе:</b> <br>
								<b>Пользотватель:</b> ' .$login.'<br>
								<b>Заказ:</b> '.$titles.'<br>
								<b>Сумма:</b> '.$summa.' руб.<br>
								<b>Телефон:</b> '.$phone.'<br>
								<b>Адрес:</b> '.$adress.' 
						</html>'; 
						$headers2  = "Content-type: text/html; charset=utf-8 \r\n"; 
						$headers2 .= "From: Суши Экспресс <vh128960@vh2.eurobyte.ru>\r\n";  
						mail($to2, $subject2, $message2, $headers2); 
						if ($result=='TRUE')
							{
								echo "<div class='alert alert-success'><strong>Заказ оформлен! </strong> Скоро с вами свяжется наш оператор и обговорит с вами время доставки.<br><br><b>На электронную почту, указанную при регистрации отправлена информация о заказе.</b></div><br><form action='/'><button class ='btn btn-success btn-group' onlick='return false;'>Переидти на главную</button></form>";
								unset($_SESSION['product_id']); 
								unset($_SESSION['product_price']); 
								unset($_SESSION['product_count']);
								unset($_SESSION['product_title']);
						...
						echo "<div class='alert alert-danger'><strong>Ошибка! </strong>Заказ не оформлен</div><form action='/'><button class ='btn btn-success btn-group' onlick='return false;'>Переидти на главную</button></form>";
						}}}}
					?>
				</div>
			</center>
