				...
				<div class="page-header">
					...
					if (empty($_SESSION['login'])) {
						if (empty($login) or empty($password)) 
							{?>
								<div class='alert alert-warning'><strong>Внимание! </strong>Вы ввели не всю информацию, вернитесь назад и заполните все поля!</div><form action='/login.php'><button class ='btn btn-success btn-group' onlick='return false;'>Вернуться назад</button></form>
							<?}else{
							...
							{
								echo ("<div class='alert alert-danger'>Извините, введённый вами логин или пароль неверны.</div><form action='/login.php'><button class ='btn btn-success btn-group' onlick='return false;'>Вернуться назад</button></form>");
							}
							else 
							{
								if ($myrow['password']==(md5($password))) {
									...
									$_SESSION['adm']=$myrow['adm'];
									$_SESSION['email']=$myrow['email'];?>
									<div class='alert alert-success'><span class="glyphicon glyphicon-refresh glyphicon-refresh-animate"></span> Входим на сайт...</div>
									<?
								}
								else 
								{
									echo "<div class='alert alert-danger'>Извините, введённый вами логин или пароль неверный.</div><form action='/login.php'><button class ='btn btn-success btn-group' onlick='return false;'>Вернуться назад</button></form>";
								}
							}
							}
					}else{
						?>
						<div class='alert alert-warning'><strong>Ошибка! </strong>Вы уже авторизованы.</div><form action='/'><button class ='btn btn-success btn-group' onlick='return false;'>Вернуться на главную</button></form>
						<?}
					?>
				</div>
			</center>
