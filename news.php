							...
							<center>
								<td><h4><?php echo $_SESSION['product_title'][$i]?></h4></td>	
									...
									<form action="<?php echo $_SERVER[‘REQUEST_URI’]?>" method="POST">
											...
											<button type="submit" class="btn btn-danger btn-xs">Удалить товар из корзины</button>
									</form>
									</label>
		...
		<div class="row">
			  <div class="col-sm-15 col-md-15">
				<h3>Новости</h3>
					<?php
						$c=0;
						$r= mysql_query ("SELECT * FROM news ORDER BY date DESC"); 
							...
							?>
								<div class="panel panel-default">
									...
									<div class="panel-body" style="background-color: #f5f5f5;">
										<h3 style="margin-top: 0px;"><?php echo $row['title']; ?></h3>
										<?php echo $row['msg']; ?>
									</div>
									<div class="panel-footer"><b>Автор: </b><?php echo $row['username']; ?> | <b>Дата: </b><?php echo $row['date']; ?>
										<?php
										if ($_SESSION['adm'] == 1) {
											?>
											<script>
											...
											<tr>
												<td>
												<form name="form2" action="save_news.php" method="post" onSubmit="return splash();">
												...
												<td>
												<form name="form3" action="editnews.php" method="post">
													<input type="hidden" name="action" value="edit">
													<input type="hidden" name="editid" value="<?php echo $row['id']; ?>">
													<input type="hidden" name="editmsg" value="<?php echo $row['msg']; ?>">
													<input type="hidden" name="edittitle" value="<?php echo $row['title']; ?>">
													<input type="hidden" name="editimg" value="<?php echo $row['img']; ?>">
													<button type="submit" class="btn btn-warning btn-xs"><span class="glyphicon glyphicon-ok"></span> Редактировать</button>
												</form>
												</td>
											</tr>
											</table>
						...
						if ($c==0)
							echo "<center>Новостей нет!<br></center>";
					?>
			  </div>
			</div>
	</div>
